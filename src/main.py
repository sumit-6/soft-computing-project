...
def infer(model: Model, fn_img: Path) -> None:
    ...
    recognized = [x.lower() for x in  recognized]
    x = np.linspace(-15, 15, 10000)
    function = recognized[0]
    if("sinx" in function):
        function = function.replace("sinx", "np.sin(x)")
    if("cosx" in function):
        function = function.replace("cosx", "np.cos(x)")
    if("tanx" in function):
        function = function.replace("tanx", "np.tan(x)")
    if("cotx" in function):
        function = function.replace("cotx", "np.cot(x)")
    if("cosecx" in function):
        function = function.replace("cosecx", "np.cosec(x)")
    if("secx" in function):
        function = function.replace("secx", "np.sec(x)")
    if("ex" in function):
        function = function.replace("ex", "np.e**x")
    if("sinx" in function):
        function = function.replace("sinx", "np.sin(x)")
    if("xx" in function):
        function = function.replace("xx", "x*x")
    y = eval(function)
    plt.plot(x, y)
